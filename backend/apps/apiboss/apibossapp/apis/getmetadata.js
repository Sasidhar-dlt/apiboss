...
exports.doService = async jsonReq => {
  ...
  if (!validateRequest(jsonReq)) { LOG.error(`Bad Request`); return { data: {result: false }} }
  else {
    ...
    if (userslist.result && userslist.users.length > 0) {
      ...
      if (result) return  { data: {result: true, data: _getMetadata(jsonReq),name:jsonReq.org,isPublic:jsonReq.isPublicServer}}
