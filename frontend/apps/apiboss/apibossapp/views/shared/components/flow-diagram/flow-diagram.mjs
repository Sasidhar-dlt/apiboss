...
const MSG_REGISTER_SHAPE = "REGISTER_SHAPE", MSG_ADD_SHAPE = "ADD_SHAPE", MSG_SHAPE_CLICKED = "SHAPE_CLICKED",
	...
	MSG_SHAPES_CONNECTED = "SHAPES_CONNECTED", GRAPH_APIBOSS_ID = "__org_monkshu_apiboss_id",
...
async function _getGraph(hostID) {
	...
	GRAPHS[hostID][GRAPH_APIBOSS_ID] = hostID; const graph = GRAPHS[hostID];
	...
	graph.addListener(mxEvent.DOUBLE_CLICK, (sender, evt) => {	// shape is double clicked
		...
		if (cell?.vertex) {
			blackboard.broadcastMessage(MSG_SHAPE_CLICKED, {
				name: cell.style, id: cell.id,
				graphID: _findGraphID(sender), label: cell.value
			}); evt.consume();
		}
	});
	...
	graph.addListener(mxEvent.CELLS_MOVED, (sender, evt) => {	// cells moved
		...
		for (const cell of cells) if (cell.vertex) blackboard.broadcastMessage(	// cell moved
			MSG_SHAPE_MOVED, {
				name: cell.style, id: cell.id, graphID: _findGraphID(sender),
	...
	mxConnectionHandler.prototype.insertEdge = function (_parent, _id, _value, source, target, _style) {
		...
		blackboard.broadcastMessage(MSG_SHAPES_CONNECTED, {
			sourceName: source.style, sourceID: source.id,
	...
	graph.addListener(mxEvent.LABEL_CHANGED, (sender, evt) => {	// label changed
		const cell = evt.getProperty("cell");
		if (cell?.vertex) blackboard.broadcastMessage(MSG_LABEL_CHANGED, {
			name: cell.style, id: cell.id,
...
function _createNonWebComponentDiagramContainer(container) {
	...
	new ResizeObserver(_ => {	// resize on resizing of the parent component
		...
		diagramContainer.style.height = `${rect.height}px`;
	}).observe(container);
...
const _findGraphID = graph => { for (const key in GRAPHS) if (GRAPHS[key][GRAPH_APIBOSS_ID] == graph[GRAPH_APIBOSS_ID]) return key; return null; }

// convert this all into a WebComponent so we can use it
export const flow_diagram = {
	trueWebComponentMode: true, registerShape, elementConnected, insertNode,
